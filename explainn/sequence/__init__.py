...
def one_hot_encode_many(seqs):
    """One hot encodes a list of sequences."""
    return(np.array([one_hot_encode(seq) for seq in seqs]))
...
def one_hot_decode_many(seqs):
    """Reverts the one hot encoding for a list of sequences."""
    return(np.array([one_hot_decode(encoded_seq) for encoded_seq in seqs]))
