...
def __get_FASTA_sequences(intervals_file, genome_file, chrom_sizes,
    ...
    # # Initialize
    # sequences = []
    # prefix = re.search("^(\S+).\S+@\S+@\S+@\w+.(\w+\-\w+\-\w+)@.\w+.peaks$",
    #     os.path.split(intervals_file)[1]).group(1)
    # print(prefix)
    # exit(0)
    ...
    for s in SeqIO.parse(dummy_file, "fasta"):
        sequences.append(SeqRecord(s.seq, id=s.id, description="0."))
