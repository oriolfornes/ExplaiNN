...
def main(**params):
    ...
    p = partial(__get_positive_sequences, output_dir=params["output_dir"])
    for _ in tqdm(pool.imap(p, reads_files), **kwargs):
        pass

def __get_positive_sequences(reads_file, output_dir="./"):
    ...
    sequences = []
    m = re.search("^(\S+)\.\S+@\S+@\S+@\w+\.(\S+)\.\S+\.fastq.gz$",
    ...
    for s in SeqIO.parse(gzip.open(reads_file, "rt"), "fastq"):
        ...
        if s.seq.count("N") > 0:
            continue
        sequences.append(SeqRecord(s.seq, id=s.id, description=1.))
...
# def __get_FASTA_sequences(reads_file, dummy_dir="/tmp/", kmer=6,
#     output_dir="./"):
